...
const weaponInput = document.getElementById("input1");
const classInput = document.getElementById("input2");
...
function removeOldWeaponData() {
    ...
    const rows = document.querySelectorAll(".fromWeaponAPI");
    for(let k = 0; k < rows.length; k++)// Remove the row from the parent (weapTable)
    weapTable?.removeChild(rows[k]);
}
function removeOldClassData() {
    const rows = document.querySelectorAll(".fromClassAPI");
    for(let k = 0; k < rows.length; k++)claTable?.removeChild(rows[k]);
}
function fetchNewWeaponData() {
    // Use the user input to control which weapons to fetch
    const weaponFilter = weaponInput?.value;
...
function fetchNewClassData() {
    const classFilter = classInput?.value;
    _axiosDefault.default.request({
        method: "GET",
        ...
        for(let k = 0; k < ru.results.length; k++){
           ...
           "params: { spellcasting_ability: classFilter }" in the .request block above
             */ if (classFilter == u.spellcasting_ability) {
                ...
                if (u.spellcasting_ability == "") spellcastCell.innerText = "None";
                else spellcastCell.innerText = u.spellcasting_ability;
...
},{"process":"jhUEF","./utils":"5By4s","./helpers/normalizeHeaderName":"adBZo","./core/enhanceError":"itUQr","./adapters/xhr":"ldm57","./adapters/http":"ldm57"}],"jhUEF":[function(require,module,exports) {
"use strict";
...
},{}]},["lpcHr","jeorp"], "jeorp", "parcelRequiree61a")
